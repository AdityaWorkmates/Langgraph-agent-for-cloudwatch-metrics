...
def _get_plot_candidates(state: State) -> list:
    """Extracts all potential data sources for plotting from the state."""
...
def _extract_and_merge_series(candidates: list) -> list:
    """Extracts and merges time series data from candidates."""
    series_found = []
...
def _create_plots(series: list) -> list:
    """Creates plots from the merged time series data."""
    plots = []
    for s in series:
        try:
...
def generate_plots(state: State) -> dict:
    ...
    candidates = _get_plot_candidates(state)
    series = _extract_and_merge_series(candidates)
    plots = _create_plots(series)
